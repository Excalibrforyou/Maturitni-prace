...
class SpravceZanrHra {
  ...
  public function vratVsechnyZanrHry() {
    ...
    if (empty($udajeZanruHry["id_hry"]&&empty($udajeZanruHry["id_zanr"])))
      Db::vloz("zanrhra", $udajeZanruHry);
    else  
      Db::zmen("zanrhra", $udajeZanruHry, 
               "WHERE id_hry = ? AND id_zanr = ?", array($udajeZanruHry["id_hry"],$udajeZanruHry["id_zanru"])); 
  }    
  
  
  public function odstranZanrHru($idHry,$idZanru) {
    Db::dotaz("
      DELETE FROM zanrhra
